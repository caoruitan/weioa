	...
	<body>
		...
		<div class="m-layout">
			...
			<div class="m-dropdown-box">
				<div class="m-box">
					...
					<c:if test='${workOrder.workOrderStatus eq "COFIRM"}'>
						...
						<a href="${basePath}/workorder/office/viewChart.htm?workOrderId=${workOrder.workOrderId}&sessionId=${workExpert.userId}&role=EXPERT&edit=true">
							<li>
								<p><img src="${workExpert.photo}" />${workExpert.name }</p>
								<p>沟通确认<span class="m-right"></span></p>
							</li>
						</a>
						<li>
						...
						<c:forEach items="${workExpertOperationalList}" var="expert">
							<a href="${basePath}/workorder/office/viewChart.htm?workOrderId=${workOrder.workOrderId}&sessionId=${expert.userId}&role=EXPERT&edit=true">
								<li>
					...
					<c:if test='${workOrder.workOrderStatus eq "APPROVAL" && workOrder.workOrderApprovalStatus eq "UNPASS"}'>
						<li>
							<p><img src="${basePath}/res/images/sj.png" />申请下队时间</p>
							<p>${workOrder.workTime}</p>
						</li>
					...
					<c:if test='${workOrder.workOrderStatus eq "APPROVAL" && workOrder.workOrderApprovalStatus eq "APPROVING"}'>
						<li>
							<p><img src="${basePath}/res/images/sj.png" />申请下队时间</p>
							<p>${workOrder.workTime}</p>
						</li>
						...
						<a href="${basePath}/workorder/office/viewChart.htm?workOrderId=${workOrder.workOrderId}&sessionId=${workExpert.userId}&role=EXPERT">
							<li>
								<p>
									<img src="${workExpert.photo}" />${workExpert.name }
									<c:if test="${workExpert.userId eq workOrder.confirmWorkExpert}">（确认下队）</c:if>
								</p>
								<p>查看沟通记录<span class="m-right"></span></p>
							</li>
						</a>
						<li>
							<p><img src="${basePath}/res/images/tx.png" />备选专家</p>
						...
						<c:forEach items="${workExpertOperationalList}" var="expert">
							<a href="${basePath}/workorder/office/viewChart.htm?workOrderId=${workOrder.workOrderId}&sessionId=${expert.userId}&role=EXPERT">
								<li>
									<p>
					...
					<c:if test='${workOrder.workOrderStatus ne "COFIRM" && workOrder.workOrderStatus ne "APPROVAL"}'>
						...
						<c:if test='${workOrder.workOrderStatus ne "CANCEL"}'>
							<li>
								<p><img src="${basePath}/res/images/sj.png" />下队时间</p>
								<p>${workOrder.confirmWorkTimeStart}&nbsp;&nbsp;至&nbsp;&nbsp;${workOrder.confirmWorkTimeEnd}</p>
							</li>
			...
			<c:if test='${workOrder.workOrderStatus eq "REPORT_SUBMIT" || workOrder.workOrderStatus eq "OVER"}'>
				...
				<div class="m-box-title">报告审批记录<span class="m-dropdown"></span></div>
				<div class="m-dropdown-box">
					<div class="m-box">
						<c:forEach items="${reportApprovalRecordList}" var="record">
							<li class="m-auto-height">
								<p>${record.recordContent}&nbsp;&nbsp;</p>
								<a class="zong">[${record.recordUserName}]&nbsp;<fmt:formatDate value="${record.recordTime}" type="both" dateStyle="medium" timeStyle="medium"/></a>
							</li>
		...
		<div class="m-footer">
			...
			<c:if test='${workOrder.workOrderStatus eq "COFIRM"}'>
				<span class="m-btn w-40"><a href="${basePath}/workorder/office/toConfirmWorkOrder.htm?workOrderId=${workOrder.workOrderId}" class="bg-blue">确认工作单</a></span>
				<span class="m-btn w-40"><a href="${basePath}/workorder/office/cancelWorkOrder.htm?workOrderId=${workOrder.workOrderId}" class="bg-blue">取消工作单</a></span>
				<span class="m-btn w-20"><a href="${basePath}/workorder/office/workOrderList.htm" class="bg-gray">返回</a></span>
			</c:if>
			<c:if test='${workOrder.workOrderStatus eq "APPROVAL" && workOrder.workOrderApprovalStatus eq "UNPASS"}'>
				<span class="m-btn w-60"><a href="${basePath}/workorder/office/toConfirmWorkOrder.htm?workOrderId=${workOrder.workOrderId}" class="bg-blue">确认工作单</a></span>
				<span class="m-btn w-40"><a href="${basePath}/workorder/office/workOrderList.htm" class="bg-gray">返回工作台</a></span>
			</c:if>
			<c:if test='${workOrder.workOrderStatus eq "APPROVAL" && workOrder.workOrderApprovalStatus ne "UNPASS"}'>
				<span class="m-btn w-100"><a href="${basePath}/workorder/office/workOrderList.htm" class="bg-gray">返回工作台</a></span>
			</c:if>
			...
			<c:if test='${workOrder.workOrderStatus eq "REPORT_SUBMIT" && workOrder.workOrderDoneApprovalStatus eq "APPROVING"}'>
				<span class="m-btn w-60"><a href="${basePath}/workorder/office/toApprovalReport.htm?workOrderId=${workOrder.workOrderId}" class="bg-blue">报告审批</a></span>
				<span class="m-btn w-40"><a href="${basePath}/workorder/office/workOrderList.htm" class="bg-gray">返回工作台</a></span>
			</c:if>
			<c:if test='${workOrder.workOrderStatus eq "REPORT_SUBMIT" && workOrder.workOrderDoneApprovalStatus ne "APPROVING"}'>
				<span class="m-btn w-100"><a href="${basePath}/workorder/office/workOrderList.htm" class="bg-gray">返回工作台</a></span>
			</c:if>
