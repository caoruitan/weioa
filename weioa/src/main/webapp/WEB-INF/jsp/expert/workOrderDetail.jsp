	...
	<body>
		...
		<div class="m-layout">
			...
			<div class="m-dropdown-box">
				<div class="m-box">
					...
					<li>
						<p><img src="${basePath}/res/images/tx.png" />下队专家</p>
						<p>${workOrder.confirmWorkExpertName}</p>
					</li>
				</div>
		...
		<div class="m-footer">
			<c:if test='${workOrder.workOrderStatus eq "DOING"}'>
				<span class="m-btn w-40"><a href="${basePath}/workorder/expert/toAddDailyLog.htm?workOrderId=${workOrder.workOrderId}" class="bg-blue">填写工作日志</a></span>
				<span class="m-btn w-30"><a href="${basePath}/workorder/expert/doneWorkOrder.htm?workOrderId=${workOrder.workOrderId}" class="bg-blue">工作完成</a></span>
				<span class="m-btn w-30"><a href="${basePath}/workorder/expert/workOrderList.htm" class="bg-gray">返回工作台</a></span>
			</c:if>
			<c:if test='${workOrder.workOrderStatus eq "DONE"}'>
				<span class="m-btn w-60"><a href="${basePath}/workorder/expert/toSubmitReport.htm?workOrderId=${workOrder.workOrderId}" class="bg-blue">上传工作报告/财务报告</a></span>
				<span class="m-btn w-40"><a href="${basePath}/workorder/expert/workOrderList.htm" class="bg-gray">返回工作台</a></span>
			</c:if>
			<c:if test='${workOrder.workOrderStatus eq "REPORT_SUBMIT" && workOrder.workOrderDoneApprovalStatus eq "UNPASS"}'>
				<span class="m-btn w-60"><a href="${basePath}/workorder/expert/toSubmitReport.htm?workOrderId=${workOrder.workOrderId}" class="bg-blue">重新上传工作报告/财务报告</a></span>
				<span class="m-btn w-40"><a href="${basePath}/workorder/expert/workOrderList.htm" class="bg-gray">返回工作台</a></span>
			</c:if>
			<c:if test='${workOrder.workOrderStatus eq "REPORT_SUBMIT" && workOrder.workOrderDoneApprovalStatus ne "UNPASS"}'>
				<span class="m-btn w-100"><a href="${basePath}/workorder/expert/workOrderList.htm" class="bg-gray">返回工作台</a></span>
			</c:if>
